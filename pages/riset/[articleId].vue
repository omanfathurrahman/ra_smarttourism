...
const route = useRoute('riset-articleId')
...
async function fetchArticle() {
  ...
  console.log(articleDocumentationsResponse)
}
...
<template>
  <div>
    ...
    <div class="lg:px-[6rem] px-4 py-4 overflow-hidden text-black xl:pb-24" v-if="curArticle">
      ...
      <div class="relative mt-8 xl:mt-12">
        ...
        <NuxtImg class="w-full aspect-video object-cover overflow-hidden rounded-lg mt-4" :src="curArticle?.img_url!" />
      </div>
      ...
      <div class="mt-4 xl:mt-8">
        ...
        <div class="text-justify flex flex-col gap-3 mt-3">
          <ArticleDescriptionOrWhatWeDid v-if="curArticleDescriptionContentList"
      ...
      <div class="mt-4 xl:mt-8">
        ...
        <ArticleDescriptionOrWhatWeDid v-if="curArticleWhatWeDidContentList"
      ...
      <div class="mt-4 xl:mt-8">
        ...
        <div class="px-14 space-y-4" v-if="curArticleDocumentationImgs">
          <div class="lg:even:flex-row-reverse lg:even:pl-[25%] lg:odd:flex-row lg:odd:pr-[25%] space-y-4 lg:space-y-0 lg:flex lg:h-72 gap-4"
               v-for="i in Math.ceil(curArticleDocumentationImgs?.length! / 2)" :key="i">
            <div class="w-full h-full object-center" v-for="img_url in getImgSliced(i)">
              <div class="w-full h-full overflow-hidden shadow-lg rounded-md hover:shadow-2xl transition-all hover:z-40 hover:scale-105" v-if="img_url">
                <NuxtImg v-if="img_url" :src="img_url" :key="img_url" class="h-full w-full object-cover" />
              </div>
            </div>
          </div>
        </div>
