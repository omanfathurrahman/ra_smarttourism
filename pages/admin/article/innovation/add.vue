...
import type { ArticleDocumentationImgUrl } from '~/types/Article';
  ...
  const uploadedPosterImgSrc = ref<ImageUploaded>()
  ...
  async function uploadPosterImageToS3() {
    const { img_url, img_path, img_url_expired_time } = await uploadInnovationPosterArticleImage({
  ...
  async function uploadDocumatationImagesToS3() {
    for (let i = 0; i < uploadedDocumentationImgSrc.value.length; i++) {
      const url = await uploadInnovationDocumentationArticleImage({
...
<template>
  <div class="h-full flex flex-col gap-3">
    <h1 class="text-2xl">Tambah Artikel Inovasi</h1>
    <form
