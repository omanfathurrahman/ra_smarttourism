...
const uploadResearchPosterArticleImage = async ({ file, articleName, imageName }: { file: File, articleName: string, imageName: string }) => {
  const ext = file?.type.split("/")[1]
  const judulImage = imageName.replaceAll(" ", "-")
  const judulArticle = articleName.replaceAll(" ", "-")
  const fileName = `article/research/${judulArticle}/poster/${judulImage}.${ext}`
...
export { uploadResearchPosterArticleImage }
